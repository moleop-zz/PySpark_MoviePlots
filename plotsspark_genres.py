...
filter_words = sc.textFile("movieplots/filter_list.txt")
filter_list = filter_words.flatMap(lambda line: line.split(" ")).map(lambda x: (x,1))
relevant_words = wcount.subtractByKey(filter_list)
relevant_words.takeOrdered(150, lambda (word,count): -count)

output = str(relevant_words.takeOrdered(50, lambda (word,count): -count))
